      ...
      <el-dialog v-model="dialogAdd" :title="dialogTitle" width="500" :show-close="false" :destroy-on-close="true"
        ...
        <el-form :inline="true" ref="formref" id="form" :model="dialogForm" size="large" label-width="100px"
          ...
          <el-form-item label="省" prop="province" label-width="130px">
            <el-input class="search-input" clearable placeholder="" :maxlength="50" :disabled="dialogFormDisabled"
...
const getUserPower = () => {
  return basicService.getPower(
    ...
    .then((res) => {
      // console.log(res)
      if (res.data.findIndex(item => item.menu_index == 'province_add') !== -1) addPower.value = true
      if (res.data.findIndex(item => item.menu_index == 'province_edit') !== -1) editPower.value = true
      if (res.data.findIndex(item => item.menu_index == 'province_del') !== -1) deletePower.value = true
    })
    .catch((error) => {
      console.log(error)
    })
